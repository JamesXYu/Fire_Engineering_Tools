...
### Help Windows (Detail Windows)
- Automatically positioned next to the calculator window
- Cannot be dragged independently (attached to source window)
- Fixed size and non-movable
- Title can be customized (e.g., "Justification" for mergeflow)
- Can show method-specific content for multi-method calculators
- Mutually exclusive with figure windows (only one can be open at a time)

### Figure Windows
- **Figure Button**: Each calculator window has a 🖼️ button in the top-left corner of the title bar
- Opens a window displaying an image from the `Figures` folder
- Image path format: `Figures/{calculator-type}.png` (e.g., `Figures/mergeflow.png`)
- For multi-method calculators: `Figures/{calculator-type}-{method}.png` (e.g., `Figures/BSmergeflow-method1.png`)
- Automatically positioned next to the calculator window
- Cannot be dragged independently (attached to source window)
- Fixed size (600x500) and non-resizable
- Mutually exclusive with help/detail windows (only one can be open at a time)
- For multi-method calculators, the figure window automatically updates when the method changes
